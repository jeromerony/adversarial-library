...
_attacks_untargeted_minimal_linf = (
    partial(attacks.carlini_wagner_linf, max_iterations=20, learning_rate=0.1),
    partial(attacks.df, steps=10, norm=float('inf')),
    partial(attacks.fab, norm=float('inf'), n_iter=10),
    partial(attacks.fmn, norm=float('inf'), steps=10),
    partial(attacks.pdpgd, norm=float('inf'), num_steps=200),
    partial(attacks.tr, iter=50, eps=0.1, p=float('inf')),
)


@pytest.mark.parametrize('attack', _attacks_untargeted_minimal_linf)
@pytest.mark.parametrize('batch_size', [1, 3, 8])
@pytest.mark.parametrize('dims', ((8,), (4, 6), (5, 7, 7)))
def test_untargeted_minimal_linf(attack: Callable, batch_size: int, dims: tuple[int]):
    model = Linear(input_dim=math.prod(dims))
    inputs = torch.randn(batch_size, *dims).mul_(0.03).add_(0.75).clamp_(min=0, max=1)
    labels = inputs.new_ones(batch_size, dtype=torch.long)
    ...
    # weight is uniform (w_i = 1/d), so the L2-optimal projection direction is also
    # the L_inf-optimal one: model.project lands on the same boundary point either way.
    projection = model.project(inputs)
    pred_projection = model(projection)[..., :2]
    torch.testing.assert_close(pred_projection, torch.zeros_like(pred_projection))

    best_norm = (projection - inputs).flatten(1).norm(p=float('inf'), dim=1)
    adv_norm = (adv_inputs - inputs).flatten(1).norm(p=float('inf'), dim=1)
    ratio = adv_norm / best_norm
    logger.info(f"Ratio of adv norm over best: {ratio.numpy(force=True)}")
    assert (ratio <= 1.25).all()


_attacks_untargeted_budget_linf = (
    (partial(attacks.pgd_linf, steps=10, absolute_step_size=0.1), 'ε'),
    (partial(attacks.apgd, norm=float('inf'), n_iter=10), 'eps'),
)


@pytest.mark.parametrize('attack,budget_kw', _attacks_untargeted_budget_linf)
@pytest.mark.parametrize('batch_size', [1, 3, 8])
@pytest.mark.parametrize('dims', ((8,), (4, 6), (5, 7, 7)))
def test_untargeted_budget_linf(attack: Callable, budget_kw: str, batch_size: int, dims: tuple[int]):
    model = Linear(input_dim=math.prod(dims))
    inputs = torch.randn(batch_size, *dims).mul_(0.01).add_(0.75).clamp_(min=0, max=1)
    labels = inputs.new_ones(batch_size, dtype=torch.long)
    ...
    budget = (projection - inputs).flatten(1).norm(p=float('inf'), dim=1).max().item() * 1.25
...
_attacks_targeted_minimal_linf = (
    partial(attacks.carlini_wagner_linf, max_iterations=20, learning_rate=0.1),
    partial(attacks.fmn, norm=float('inf'), steps=10),
    partial(attacks.pdpgd, norm=float('inf'), num_steps=200),
)
...
@pytest.mark.parametrize('attack', _attacks_targeted_minimal_linf)
@pytest.mark.parametrize('batch_size', [1, 3, 8])
@pytest.mark.parametrize('dims', ((8,), (4, 6), (5, 7, 7)))
def test_targeted_minimal_linf(attack: Callable, batch_size: int, dims: tuple[int]):
    torch.manual_seed(0)
    model = Linear(input_dim=math.prod(dims))
    inputs = torch.randn(batch_size, *dims).mul_(0.03).add_(0.75).clamp_(min=0, max=1)
    labels = inputs.new_ones(batch_size, dtype=torch.long)
...
_attacks_targeted_budget_linf = (
    (partial(attacks.apgd, norm=float('inf'), n_iter=10), 'eps'),
)


@pytest.mark.parametrize('attack,budget_kw', _attacks_targeted_budget_linf)
@pytest.mark.parametrize('batch_size', [1, 3, 8])
@pytest.mark.parametrize('dims', ((8,), (4, 6), (5, 7, 7)))
def test_targeted_budget_linf(attack: Callable, budget_kw: str, batch_size: int, dims: tuple[int]):
    torch.manual_seed(0)
    model = Linear(input_dim=math.prod(dims))
    inputs = torch.randn(batch_size, *dims).mul_(0.01).add_(0.75).clamp_(min=0, max=1)
    labels = inputs.new_ones(batch_size, dtype=torch.long)
