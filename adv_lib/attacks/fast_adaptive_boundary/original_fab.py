...
def fab(model: nn.Module,
    ...
    attack = FABAttack(predict=model, device=inputs.device, norm=norm, **kwargs)
    if targeted_variant:
        best_adv = inputs.clone()
        best_norm = torch.full_like(labels, float('inf'), dtype=torch.float)
        for target_class in range(2, n_target_classes + 2):
            attack.target_class = target_class
            adv_inputs_run = attack.attack_single_run_targeted(x=inputs, y=labels)
            delta_adv = (adv_inputs_run - inputs).flatten(1)
            if norm == 'L1':
                norms = delta_adv.norm(p=1, dim=1)
            elif norm == 'L2':
                norms = delta_adv.norm(p=2, dim=1)
            elif norm == 'Linf':
                norms = delta_adv.norm(p=float('inf'), dim=1)
            smaller_norm = norms < best_norm
            is_adv = model(adv_inputs_run).argmax(1) != labels
            is_both = is_adv & smaller_norm
            best_adv[is_both] = adv_inputs_run[is_both]
            best_norm[is_both] = norms[is_both]
        return best_adv
    else:
        return attack.attack_single_run(x=inputs, y=labels)
